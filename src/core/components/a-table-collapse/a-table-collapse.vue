            ...
            <template v-for="(row, index) in rows">
              ...
              >
                <td v-if="selectOptions.enable && false" class="selection">
                  <input
                    type="checkbox"
                    :checked="row[selectOptions.label]"
                ...
                >
                  <span class="icon" :class="{ colapsed: row.isOpen }">
                    <slot name="colapse-icon"> ^ </slot>
                  </span>
                </td>
              </tr>
              <a-table-colabse-child
                v-if="colapseOptions.enable"
              ...
              >
                ...
                <template
                  ...
                  slot-scope="{ tr, trIndex }"
                >
                  <slot
                    :name="'item-th.' + th.value"
                    :tr="tr"
                    :trIndex="trIndex"
                    :parent="row"
                  ></slot>
...
<script>
import aTableColabseChild from "./a-table-colabse-child.vue";
...
export default {
  ...
  watch: {
    ...
    innerValue(val) {
      ...
      this.rows.forEach((row) => {
        row[this.childrenLabel].forEach((child) => {
          if (this.value.indexOf(child.id) != -1)
            this.$set(child, this.selectedLabel, true);
        });
      });
    },
    rows() {
